...
def test_adaptive_stale_timeout_with_frequent_updates(bermuda_advert: BermudaAdvert) -> None:
    ...
    with patch("custom_components.bermuda.bermuda_advert.monotonic_time_coarse", return_value=base_time + 61):
        bermuda_advert.calculate_data()
        # rssi_distance is CLEARED so stale incumbents don't block fresh challengers
        # in distance comparisons. Velocity-related history is also cleared.
        assert bermuda_advert.rssi_distance is None  # Cleared!
...
class TestBermudaAdvertEdgeCases:
    ...
    def test_clear_stale_history(self, bermuda_advert: BermudaAdvert) -> None:
        """Test _clear_stale_history clears rssi_distance and velocity-related history.

        rssi_distance is cleared so that stale adverts don't block fresh challengers
        in distance comparisons. In _refresh_area_by_min_distance, distance comparison
        happens BEFORE variance-based checks, so a stale incumbent with a small old
        distance could otherwise beat fresh challengers indefinitely.
        """
        bermuda_advert.rssi_distance = 5.0
        ...
        # rssi_distance is CLEARED so stale adverts don't block fresh challengers
        assert bermuda_advert.rssi_distance is None
        # rssi_filtered is preserved (Kalman state, used for variance calculation)
